...
## OpenMint: Загрузчик профилей и медиа из соцсетей

Скачивайте полные профили авторов, галереи изображений, видеоролики и истории с популярных социальных платформ. Работающий на базе `gallery-dl`, OpenMint выполняет загрузки асинхронно и управляет ими в отдельной удобной вкладке.

- **Поддерживаемые платформы:** Instagram, TikTok, Facebook и X (бывший Twitter).
- **Управление профилями:** Сохраняйте ссылки на профили любимых авторов, чтобы проверять наличие обновлений и скачивать новый контент в один клик.
- **Истории и актуальное:** Скачивайте активные истории (Stories) и актуальное (Highlights) в Instagram с помощью встроенного помощника.
- **Дополнительные настройки:** Задавайте собственную папку вывода, загружайте файлы cookies для скачивания приватного или ограниченного контента, направляйте трафик через прокси, задавайте свой User-Agent и выбирайте тип контента (только изображения, только видео или всё сразу).
- **Прогресс в реальном времени:** Живой счётчик файлов, дедупликация и потоковый лог, чтобы вы всегда знали, что происходит.
- **Асинхронное глобальное состояние:** Скачивание происходит в фоновом режиме, что позволяет вам переходить в другие разделы приложения, не теряя информацию о ходе загрузки и лог прогресса.
...
OmniGet поставляется с полным набором плагинов (курсы, обучение, Telegram, конвертация и другие), и они настраиваются сами при первом запуске. Они также сами обновляются, когда выходит новая версия, так что вам не нужно ничего искать и качать. Плагины подключаются «на лету» без перезапуска приложения. Включайте и выключайте любой из них в боковой панели и удаляйте ненужные. Что вы удалили, то и остаётся удалённым.

Совместимость ABI плагинов проверяется при загрузке — каждый плагин указывает версию SDK, и ядро отклоняет несовместимые бинарники с понятным сообщением об ошибке, чтобы сломанный плагин никогда не работал молча.
...
- **10 языков** с полным покрытием интерфейса — каждая кнопка, подпись и подсказка переведены, файлы локалей синхронизируются автоматически, ни один ключ не пропущен. **15 тем**, включая Catppuccin, Dracula, One Dark Pro, NyxVamp и три варианта для электронных чернил.
- **Поддержка токена GitHub** в экспертных настройках для обхода лимитов GitHub API при проверке обновлений.
...
**Что такое OpenMint?**
OpenMint — это встроенный в OmniGet менеджер для скачивания контента из социальных сетей. Используя `gallery-dl` под капотом, он позволяет скачивать целые галереи, видео, истории и актуальное из Instagram, TikTok, Facebook и X. Вы также можете сохранять профили авторов для удобного доступа.
...
Если вы представляете указанную платформу и у вас есть вопросы, [откройте issue](https://github.com/sayfalse/omniget/issues) или напишите с корпоративного адреса. Платформа будет убрана из списка сразу же.
