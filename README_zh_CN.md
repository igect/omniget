...
## OpenMint：创作者与社交媒体下载器

下载主流社交媒体平台上的完整创作者主页、图集、视频和快拍。OpenMint 底层基于 `gallery-dl` 驱动，采用异步运行，并在独立的标签页中管理您的下载。

- **支持的平台**：Instagram、TikTok、Facebook 和 X（前 Twitter）。
- **主页管理**：保存您喜爱的创作者主页/链接，一键检查并下载新内容。
- **快拍与精选**：通过集成的助手轻松下载 Instagram 的动态快拍（Stories）和精选（Highlights）。
- **高级选项**：支持设置自定义输出目录、加载 Cookie 文件以下载私密或受限内容，并可自由选择仅下载图片、视频或所有媒体类型。
- **异步全局状态**：下载在后台静默运行，您可以自由切换到应用的其他界面，下载状态或进度日志不会丢失。
...
**什么是 OpenMint？**
OpenMint 是 OmniGet 内置的社交媒体与创作者内容下载管理器。它在底层使用 `gallery-dl`，支持从 Instagram、TikTok、Facebook 和 X 下载完整的图集、视频、快拍和精选。您可以保存创作者主页以方便后续下载。
