...
async fn run_import(slug: String, kind: UrlKind) -> Result<BilibiliImportResult, String> {
    ...
    loop {
        ...
        let parsed = match kind_ref {
            UrlKind::WatchLater => {
                omniget_core::platforms::bilibili::parser::watch_later::parse(&client, current_page).await
            }
            UrlKind::History => {
                omniget_core::platforms::bilibili::parser::history::parse(&client, current_page).await
            }
