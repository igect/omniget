<script lang="ts">
  ...
  async function render(src: string) {
    ...
    try {
      ...
      if (token !== renderToken) return;
      if (!svgHost) {
        renderState = { kind: "error", message: t("study.notes.fc_no_container") };
        return;
      }
...
<div class="flowchart-block" data-flowchart data-mode={mode}>
  <header class="flowchart-head" contenteditable="false">
    ...
    <span class="flowchart-label">{t("study.notes.fc_label")}</span>
    <button
      type="button"
  ...
  {#if mode === "source"}
    <textarea
      ...
      aria-label={t("study.notes.fc_source_aria")}
    ></textarea>
  {:else if renderState.kind === "idle"}
    <p class="flowchart-state">{t("study.notes.fc_no_source")}</p>
  {:else if renderState.kind === "loading"}
    <p class="flowchart-state">{t("study.notes.rendering")}</p>
  {:else if renderState.kind === "error"}
    <div class="flowchart-error">
      <p class="flowchart-error-msg">{t("study.notes.error_prefix")}{renderState.message}</p>
