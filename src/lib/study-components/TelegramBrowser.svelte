<script lang="ts">
  ...
  async function selectChat(chat: TelegramChat) {
    ...
    if (mediaItems.length === 0 && !mediaError) {
      console.warn(
        `[TG] selectChat: chat="${chat.title}" type=${chat.chat_type} returned 0 media even with filter=all — running diag`,
      );
      try {
        ...
        if (diag.get_history_error) {
          mediaError = $t("study.telegram.browser.diag_access_denied", { error: diag.get_history_error });
        } else if (diag.search_errors.length > 0) {
          mediaError = $t("study.telegram.browser.diag_search_errors", { errors: diag.search_errors.join("; ") });
        } else if (diag.get_history_count === 0 && diag.search_photo_count === 0 && diag.search_video_count === 0 && diag.search_document_count === 0) {
          mediaError = $t("study.telegram.browser.diag_no_accessible_media");
        } else if (diag.get_history_with_media === 0) {
          mediaError = $t("study.telegram.browser.diag_no_detectable_media", { count: diag.get_history_count });
        }
      } catch (e) {
  ...
  async function loadMedia(reset: boolean) {
    ...
    try {
      ...
      const timeoutPromise = new Promise<TelegramMediaItem[]>((_, reject) =>
        setTimeout(
          () => reject(new Error($t("study.telegram.browser.diag_timeout", { ms: timeoutMs }))),
          timeoutMs,
        ),
      );
  ...
  async function bulkBookmark() {
    ...
    try {
      ...
      // L11: auto-tag in background — non-blocking
  ...
  function onKeyDown(ev: KeyboardEvent) {
    ...
    if ((ev.ctrlKey || ev.metaKey) && ev.key.toLowerCase() === "f" && !inEditableField) {
      const sel = selectedChat
        ? '.tg-browser input.search-input[data-search-context="media"]'
        : '.tg-browser input.search-input[data-search-context="chat"]';
